...
export const NewsPost: React.FC<NewsPostProps> = ({ content, format, showLogo, scale = 1 }) => {
  ...
  // Ajuste automático de fonte para títulos longos
  ...
  const containerStyle: React.CSSProperties = {
    ...
    position: 'relative',
    overflow: 'hidden',
    backgroundColor: '#050a1d', // Azul Marinho Profundo
    color: 'white'
  };
    ...
    <div style={containerStyle} className="flex flex-col font-sans">
        ...
        {showLogo && (
          <div className="absolute top-14 right-14 z-20 drop-shadow-[0_10px_20px_rgba(0,0,0,0.6)]">
            {content.logoUrl ? (
              ...
              <div className="bg-white text-blue-900 p-6 font-black text-3xl rounded shadow-2xl">
                LOGO
              </div>
            )}
          </div>
      ...
      <div className={`${isFeed ? 'h-[38%]' : 'h-[28%]'} w-full p-20 flex flex-col justify-end bg-[#050a1d] relative z-10`}>
        <div className="mb-8 flex items-center gap-6">
          <span className="bg-red-600 text-white px-6 py-2.5 text-2xl font-black uppercase tracking-[0.2em] shadow-xl">
            Urgente
          </span>
          <div className="h-[2px] flex-grow bg-white/20" />
        </div>
        
        <h1 className={`font-news ${titleClass} font-black leading-[1.05] mb-10 uppercase tracking-tight drop-shadow-lg`}>
          {content.title}
        </h1>
